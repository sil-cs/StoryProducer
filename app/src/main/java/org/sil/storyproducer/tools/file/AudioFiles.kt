...
fun createRecordingCombinedName() : String {
    ...
    return when(Workspace.activePhase.phaseType) {
        ...
        PhaseType.LEARN, PhaseType.WHOLE_STORY -> {
            "${Workspace.activePhase.getDirectoryName()}|$PROJECT_DIR/${Workspace.activePhase.getShortDirectoryName()}$AUDIO_EXT"
        }
        ...
        PhaseType.VOICE_STUDIO, PhaseType.ACCURACY_CHECK -> {
            ...
            val rNameNum = "${Workspace.activePhase.getDirectoryName()} ([0-9]+)".toRegex()
            var maxNum = 0
            for (n in names!!){
                try {
            ...
            "${Workspace.activePhase.getDirectoryName()} ${maxNum+1}|${Workspace.activeDir}/${Workspace.activeFilenameRoot}_${Date().time}$AUDIO_EXT"
        }
        else -> {""}
    }
}
