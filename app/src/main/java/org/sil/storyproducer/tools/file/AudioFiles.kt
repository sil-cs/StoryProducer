...
fun createRecordingCombinedName() : String {
    ...
    return when(Workspace.activePhase.phaseType) {
        ...
        PhaseType.LEARN, PhaseType.WHOLE_STORY -> {
            "${Workspace.activePhase.getFileSafeName()}|$PROJECT_DIR/${Workspace.activePhase.getFileSafeName()}$AUDIO_EXT"
        }
        ...
        PhaseType.VOICE_STUDIO, PhaseType.ACCURACY_CHECK -> {
            ...
            val rNameNum = "${Workspace.activePhase.getFileSafeName()} ([0-9]+)".toRegex()
            var maxNum = 0
            for (n in names!!){
                try {
            ...
            "${Workspace.activePhase.getFileSafeName()} ${maxNum+1}|${Workspace.activeDir}/${Workspace.activeFilenameRoot}_${Date().time}$AUDIO_EXT"
        }
        else -> {""}
    }
}
